...
void CurrentSTL::insertItem(const QString &newName, const QModelIndex &index)
{
    ...
    //how does the user select this, where they currently are?
    ...
    //some code to add a row at a cetain index

    //create iterator pointing to certain position
    //std::vector<StockItem>::iterator itPos; //use auto instead?

    auto itPos = modelNames.begin() + index.row()+1;
    itPos = modelNames.insert(itPos, newName);
    
    //stockItems.insert(index, s);
